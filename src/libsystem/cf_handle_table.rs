...
/// Intern a new host CF pointer, returning a stable 32-bit handle.
/// Deduplicates: the same pointer always returns the same handle.
/// NULL → 0.
pub fn cf_intern(ptr: *const c_void) -> u32 {
    if ptr.is_null() {
        return 0;
    }
    ...
    TABLE.with(|t| {
        ...
        // Check again in case of re-entry (shouldn't happen but be safe)
...
/// Resolve a value returned from a host CF function back to a 32-bit guest value.
///
/// - Already in the handle table → return its handle (same pointer = same handle).
/// - Fits in 32 bits (was a raw i386 value stored verbatim by CF) → return as-is.
/// - New 64-bit host pointer not yet tracked → intern and return a new handle.
pub fn cf_result(ptr: *const c_void) -> u32 {
    if ptr.is_null() {
        return 0;
    }
    ...
    if raw <= 0xFFFF_FFFF {
        // Raw i386 value stored in a CF container; return it directly.
        return raw as u32;
    }
    cf_intern(ptr)
}
...
///
/// - 0 → NULL.
/// - Known handle → real host CF pointer.
/// - Unknown (raw i386 address) → zero-extended to 64 bits as a *const c_void.
pub fn cf_arg(v: u32) -> *const c_void {
    if v == 0 {
        return std::ptr::null();
    }
    TABLE.with(|t| {
...
/// Mutable variant of [`cf_arg`].
pub fn cf_arg_mut(v: u32) -> *mut c_void {
    cf_arg(v) as *mut c_void
}

/// Push a pending (applier_addr, element_value, context) onto the apply queue.
...
/// Pop the next pending apply item, or `None` if the queue is empty.
